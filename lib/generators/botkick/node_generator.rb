  ...
  module Generators # :nodoc:
    class NodeGenerator < ::Rails::Generators::NamedBase # :nodoc:
      ...
      def create_node_file
        ...
        template "messenger/#{@options["view"]}.yml.erb", File.join('app/bots', "#{class_path[0]}_bot/template", "#{file_name}.yml")
        template 'node.rb.erb', File.join('app/bots', "#{class_path[0]}_bot/node", "#{file_name}.rb")
      end
